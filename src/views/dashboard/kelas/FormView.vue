    ...
    <div class="max-w-3xl mx-auto space-y-6">
        <div class="flex items-center justify-between bg-white p-6 rounded-2xl shadow-sm border border-emerald-100">
            <div class="flex items-center gap-4">
                <button @click="$router.push('/dashboard/kelas')"
                ...
                <div>
                    <h1 class="text-2xl font-bold text-emerald-900">{{ isEdit ? 'Edit Data Kelas' : 'Tambah Kelas Baru'
                        }}</h1>
                    <p class="text-sm text-emerald-600">{{ isEdit ? 'Perbarui informasi kelas yang sudah ada' :
                        'Masukkan informasi kelas baru ke dalam sistem' }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-emerald-100 p-6 sm:p-8">
            ...
            <form v-else @submit.prevent="handleSubmit" class="space-y-6">
                ...
                <div>
                    <label class="block text-sm font-medium text-emerald-900 mb-1.5">Kode Kelas <span
                            class="text-red-500">*</span></label>
                    <input v-model="form.kode_kelas" type="text" required maxlength="10" placeholder="Misal: X-IPA-1"
                    ...
                    <p class="text-xs text-emerald-600/70 mt-1.5">Maksimal 10 karakter, harus unik (tidak boleh sama
                        dengan kelas lain).</p>
                </div>

                <div>
                    <label class="block text-sm font-medium text-emerald-900 mb-1.5">Nama Kelas <span
                            class="text-red-500">*</span></label>
                    <input v-model="form.nama_kelas" type="text" required maxlength="50"
                ...
                <div class="flex justify-end gap-3 pt-6 border-t border-emerald-50">
                    <button type="button" @click="$router.push('/dashboard/kelas')"
...
// HATEOAS State
const submitUrl = ref('/kelas'); // Default endpoint untuk Create
const submitMethod = ref('POST'); // Default method untuk Create
...
onMounted(async () => {
    // Cek apakah URL memiliki parameter ID (berarti mode Edit)
    if (route.params.id) {
        isEdit.value = true;
        isFetching.value = true;
        try {
            // Panggil detail data
            const res = await api.get(`/kelas/${route.params.id}`);
            const data = res.data.data;

            // Isi form
            form.value.kode_kelas = data.kode_kelas;
            form.value.nama_kelas = data.nama_kelas;

            // HATEOAS: Cari link update dari API tanpa hardcode endpoint edit
            const updateLink = data._links.find(link => link.rel === 'update');
            if (updateLink) {
                submitUrl.value = updateLink.href;
                submitMethod.value = updateLink.method; // Akan otomatis menjadi 'PUT'
            }
        } catch (error) {
            alert('Gagal mengambil data kelas atau data tidak ditemukan.');
...
const handleSubmit = async () => {
    ...
    try {
        // Eksekusi API secara dinamis menyesuaikan method (POST / PUT) dan URL
        await api({
        ...
        // Kembali ke index setelah sukses
        router.push('/dashboard/kelas');
    } catch (error) {
