<template>
    <div class="space-y-6">
        <div
            ...
            <div>
                <h1 class="text-2xl font-bold text-emerald-900">Data Mata Pelajaran</h1>
                <p class="text-sm text-emerald-600">Manajemen mata pelajaran yang diajarkan</p>
            </div>

            <div class="flex flex-wrap items-center gap-3">
                <div class="relative">
                    <input v-model="searchQuery" type="text" placeholder="Cari Mapel..." @input="debounceSearch"
                ...
                <router-link to="/dashboard/mapel/create"
            ...
            <div class="overflow-x-auto flex-1">
                <table class="w-full text-left border-collapse">
                    <thead class="bg-emerald-50/50 border-b border-emerald-100">
                        <tr>
                            <th class="p-4 text-xs font-bold uppercase text-emerald-700 tracking-wider">Kode Mapel</th>
                            <th class="p-4 text-xs font-bold uppercase text-emerald-700 tracking-wider">Nama Mata
                                Pelajaran</th>
                    ...
                    <tbody class="divide-y divide-emerald-50">
                        ...
                        <tr v-else-if="!collection?.items || collection.items.length === 0">
                            <td colspan="3" class="p-12 text-center">
                                <div class="flex flex-col items-center justify-center text-emerald-600/60">
                                    ...
                                    <p class="text-lg font-bold text-emerald-900 mb-1">Tidak ada data mapel</p>
                                    <p class="text-sm text-emerald-600 max-w-sm">
                                        {{ searchQuery ? `Tidak menemukan mapel dengan kata kunci "${searchQuery}"` :
                        ...
                        <tr v-else v-for="item in collection.items" :key="item.id"
                            ...
                            <td class="p-4"><span
                                    ...
                                    item.kode_mapel }}</span></td>
                            <td class="p-4 font-medium text-emerald-900">{{ item.nama_mapel }}</td>
                            <td class="p-4">
                                <div class="flex justify-center gap-2">
        ...
        <BaseModal :show="modal.show" title="Hapus Data Mapel?"
            :message="`Anda yakin ingin menghapus mapel ${modal.data?.nama_mapel}?`" confirmText="Ya, Hapus"
...
const fetchData = async (url = '/mapel') => {
...
const handleEdit = (item) => {
    router.push(`/dashboard/mapel/edit/${item.id}`);
};
