<template>
    <div class="space-y-6">
        <div
            ...
            <div>
                <h1 class="text-2xl font-bold text-emerald-900">Data Siswa</h1>
                <p class="text-sm text-emerald-600">Manajemen data peserta didik sekolah</p>
            </div>

            <div class="flex flex-wrap items-center gap-3">
                <div class="relative">
                    <input v-model="searchQuery" type="text" placeholder="Cari NIS atau Nama..." @input="debounceSearch"
                ...
                <router-link to="/dashboard/siswa/create"
            ...
            <div class="overflow-x-auto flex-1">
                <table class="w-full text-left border-collapse whitespace-nowrap">
                    <thead class="bg-emerald-50/50 border-b border-emerald-100">
                        <tr>
                            <th class="p-4 text-xs font-bold uppercase text-emerald-700 tracking-wider">Profil Siswa
                            </th>
                    ...
                    <tbody class="divide-y divide-emerald-50">
                        <tr v-if="isLoading" v-for="i in 5" :key="`skeleton-${i}`" class="animate-pulse">
                            ...
                            <td class="p-4">
                                <div class="h-6 w-24 bg-emerald-100 rounded-full"></div>
                            </td>
                            <td class="p-4">
                                <div class="flex flex-col gap-2">
                            ...
                            <td class="p-4">
                                <div class="flex justify-center gap-2">
                                    <div class="h-9 w-9 bg-blue-50 rounded-lg"></div>
                        ...
                        <tr v-else-if="!collection?.items || collection.items.length === 0">
                            <td colspan="4" class="p-12 text-center">
                                <div class="flex flex-col items-center justify-center text-emerald-600/60">
                                    ...
                                    <p class="text-lg font-bold text-emerald-900 mb-1">Tidak ada data siswa</p>
                                    <p class="text-sm text-emerald-600 max-w-sm">
                                        {{ searchQuery ? `Tidak menemukan siswa dengan kata kunci "${searchQuery}"` :
                            ...
                            <td class="p-4">
                                <div class="flex flex-col">
                                    ...
                                    <div class="flex items-center gap-2 mt-1">
                                        <span
                                            class="font-mono text-[11px] bg-emerald-100 text-emerald-700 px-2 py-0.5 rounded border border-emerald-200">NIS:
                                            {{ item.nis }}</span>
                                        <span class="text-xs text-emerald-600 capitalize">{{ item.gender }}</span>
                                    </div>
                                </div>
                            </td>
                            <td class="p-4">
                                ...
                                <span v-else class="text-xs text-gray-500 italic">Belum ada kelas</span>
                            </td>
                            <td class="p-4">
                                <div class="flex flex-col gap-1 text-sm text-emerald-700">
                                    ...
                                    <span v-if="!item.email && !item.phone_number"
                                        class="text-xs text-gray-400 italic">-</span>
                                </div>
                            </td>
                            <td class="p-4">
                                <div class="flex justify-center gap-2">
                                    <button @click="$router.push(`/dashboard/siswa/${item.id}`)"
        ...
        <BaseModal :show="modal.show" title="Hapus Data Siswa?"
            :message="`Anda yakin ingin menghapus data siswa ${modal.data?.nama}?`" confirmText="Ya, Hapus"
            type="danger" @close="modal.show = false" @confirm="handleDelete" />
    </div>
</template>
...
const fetchData = async (url = '/siswa') => {
...
const handleEdit = (item) => {
    router.push(`/dashboard/siswa/edit/${item.id}`);
};
